/* @flow */
import type { Describer, ExpressionType, ExpressionTypeDictionary } from './types';

async function describeFieldType(field: string, describer: Describer): Promise<ExpressionType> {
  const so = await describer.describe(describer.sobject);
  const fieldDef = so.fields.find(f => f.name === field || f.relationshipName === field);
  if (!fieldDef) {
  ...
  switch (fieldDef.type) {
    ...
    case 'integer':
    case 'double':
      return { type: 'number', precision: fieldDef.precision, scale: fieldDef.scale };
    case 'boolean':
      return { type: 'boolean' };
    case 'date':
      return { type: 'date' };
    ...
    case 'reference':
      return (
        fieldDef.name === field ?
          { type: 'string' } :
        fieldDef.relationshipName === field && fieldDef.referenceTo && fieldDef.referenceTo.length === 1 ?
          { type: 'object', sobject: fieldDef.referenceTo[0], properties: {} } :
          { type: 'object', sobject: 'Name', properties: {} }
      );
    default:
      throw new Error(`describe field ${field} on ${describer.sobject} is not supported`);
  }
}
...
export async function createFieldTypeDictionary(fields: string[], describer: Describer): Promise<ExpressionTypeDictionary> {
  let dict = {};
